...
/**
 * Full-bleed WebGL "ink in water" fluid background.
 * Usage: put <FluidBackground /> as the FIRST child inside any
 * position:relative container (e.g. your hero section), then put
 * your real content after it with position:relative + z-index so it
 * sits on top. The canvas itself never blocks clicks (pointer-events:none).
 *
 * Example:
 ...
 *   </section>
 */
export default function FluidBackground({ className = "", style = {} }) {
...
/* ---------------------------------------------------------------------- */
/*  WebGL fluid engine (Pavel Dobryakov's fluid sim, MIT) — tuned for an   */
/*  oily, marbled, vivid-on-near-black look, with a strong load burst and  */
/*  an invisible auto-cursor that keeps the ink moving forever.            */
/* ---------------------------------------------------------------------- */
function fluidSimulation(canvas) {
  ...
  const displayShader = compileShader(
    ...
    `
        ...
        vec3 dewhite (vec3 c) {
            ...
            float whiteness = smoothstep(0.35, 0.85, 1.0 - clamp((maxc - minc) / max(maxc, 0.0001), 0.0, 1.0));
            return mix(c, vec3(1.0, 0.75, 0.32) * maxc, whiteness);
        }
        void main () {
            vec3 C = dewhite(texture2D(uTexture, vUv).rgb);
  ...
  let density, velocity, divergence, curl, pressure, bloom;
